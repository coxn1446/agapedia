...
- **`App.[feature].test.js`** - Feature-specific integration tests (e.g., `App.articles.test.js`, `App.search.test.js`)
...
- **`RouteName.test.js`** - Route-specific tests (e.g., `Article.test.js`, `EditArticle.test.js`)
- Test route rendering, data fetching, navigation, and route-specific logic

### Helper Tests
Tests for utility functions in `src/helpers/`:
- **`helperName.test.js`** - Helper function tests (e.g., `articleHelpers.test.js`, `searchHelpers.test.js`)
...
- **`reducerName.reducer.test.js`** - Redux reducer and action tests (e.g., `article.reducer.test.js`)
  ...
  return (url) => {
    if (url === '/api/auth/me') {
      ...
      return Promise.resolve({
        ok: false,
        status: 401,
        json: async () => ({ error: 'Not authenticated', isAuthenticated: false }),
      });
    }
...
1. **Test Behavior, Not Implementation**
   ...
   test('calls fetchArticles function', () => {
     expect(fetchArticles).toHaveBeenCalled();
   });
   
   // ✅ Good: Tests user-visible behavior
   test('displays articles list', async () => {
     render(<Articles />);
     await waitFor(() => {
...
4. **Mock External Dependencies**
   ...
   // Mock helpers
   jest.mock('../helpers/articleHelpers', () => ({
...
1. **Feature-based grouping** - Tests are grouped by feature area (auth, articles, search, etc.)
